...
def eval_all(expressions, env):
    ...
    return scheme_eval(expressions[last], env, True)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('length of formals is not equal to length of val')
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        next_target = expressions[1] 
        env.define(target, scheme_eval(next_target, env))
        return target 
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        check_formals(target)
        lambda_expression = do_lambda_form(Pair(target.second, expressions.second), env)
        env.define(target[0], lambda_expression)
        return target[0]
    else:
...
def do_and_form(expressions, env):
    ...
    if len(expressions) == 0:
        return True
    length = len(expressions) -1 
    for i in range(length):
        if scheme_false(scheme_eval(expressions[i], env)):
            return False
    return scheme_eval(expressions[length], env, True)
...
def do_or_form(expressions, env):
    ...
    length = len(expressions)-1
    for i in range(length):
        value = scheme_eval(expressions[i],env)
...
def make_let_frame(bindings, env):
    ...
    for binding in bindings:
        ...
        if len(binding.second) > 1:
            raise SchemeError("too many values to unpack")
...
def check_formals(formals):
    ...
    for i in formals:
        if not scheme_symbolp(i):
            raise SchemeError(str(i) + 'is not a symbol')
        elif i in elements:
