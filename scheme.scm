...
(define (member condition lst)
  (cond 
      ((null? lst) #f)
      ((equal? (car lst) condition) lst)
      (else (member condition (cdr lst))))
)
...
(define (list-partitions total)
    (define (partition-helper total total2) 
      (cond ((= total2 0) nil)
        ((= total 0) (list nil))
        ((< total 0) nil)
        (else (append (apply-to-all (lambda (x) (cons total2 x)) (partition-helper (- total total2) total2)) (partition-helper total (- total2 1))))))
    (remove-consecutive (partition-helper total total))
)
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
           ...
           (cons 
                (cons 'lambda 
                      (cons (car (zip values)) (analyze body))) 
